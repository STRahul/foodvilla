...
const RestaurantMenu = () => {
  ...
  const cardInfoIndex = resInfo?.cards?.findIndex((card) => {
    return card?.card?.card?.hasOwnProperty("info");
  });

  const groupedCardIndex = resInfo.cards?.findIndex((card) => {
    return card?.hasOwnProperty("groupedCard");
  });

  const { name, areaName, cuisines, avgRating, sla, totalRatingsString } =
    resInfo?.cards[cardInfoIndex]?.card?.card?.info;
